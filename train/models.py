...
class Attention(nn.Module):
    def __init__(self, hidden_size=256, dropout=0.2):
        ...
        self.attn = nn.MultiheadAttention(hidden_size, 1, dropout)
        self.dropout = nn.Dropout(p=dropout)
        self.attention_fill_value = 1e-32
    ...
    def forward(self, x, mask = None):
        ...
        if mask is not None:
            mask = ~mask
        x = x.transpose(0, 1)
        out, probs = self.attn(x, x, x, mask)
...
class CNN(nn.Module):
    ...
    def __init__(self, vocab, emb_len, hidden_size=256, n_layers=2, dropout=0.2):
        super().__init__()
        self.vocab = vocab
        self.dropout = dropout
        self.n_layers = n_layers
        ...
        self.output = nn.Linear(hidden_size, len(vocab))
        self.drop = nn.Dropout(p=dropout)
